...
<body onload="users_read()">
  ...
  <div class="container 2">
    ...
    <button type="button" class="btn btn-primary" onclick="window.open('create.html')">
      Create Users+
    </button>
    <div class="row">
      <table class="table">
...
<script>
    ...
    fetch("http://localhost/PROJECT/Project/read.php", requestOptions)
      ...
      .then((result) => {
        ...
        for (let user of jsonObj) {
          var row =
            '<tr> <th scope="row">' + user.id + '</th>  <td>' + user.fname + '</td><td>' + user.lname + '</td><td>' + user.email + '</td><td>' + user.proid + '</td><td>' + user.proname + '</td><td>' + user.indate + '</td><td>' + user.exdate + '</td><td>' + user.price + '</td><td>' + user.interest + '</td><td><a href="edit.html?id=' + user.id + '"> Edit </a></td><td><a href="#" onclick="user_delete(' + user.id + ')"> Delete </a></td> </tr>';
  ...
  var user_delete = function(id) {
    ...
    fetch("http://localhost/PROJECT/Project/delete.php", requestOptions)
      .then((response) => response.text())
      .then((result) => {
        var jsonObj = JSON.parse(result);
